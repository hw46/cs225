...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  Animation animation;
  PNG png = png_;
  for(unsigned x = 0; x < traversal_.size(); x++){
    unsigned count = 0;
    for(Traversals::ImageTraversal::Iterator y = traversal_[x] -> begin(); y != traversal_[x] -> end(); ++y){
      if(count % frameInterval == 0){
        animation.addFrame(png);
      }
      HSLAPixel& pixel = png.getPixel((*y).x, (*y).y);
      pixel = colorPicker_[x] -> getColor((*y).x, (*y).y);
      count+=1;
    }
  }
  animation.addFrame(png);
